...
# These tests require the service with access to Khulnasoft Deployments.
# Set KHULNASOFT_ACCESS_TOKEN to an access token with access to Khulnasoft Deployments
# and set KHULNASOFT_TEST_DEPLOYMENTS_API to any value to enable the tests.
@pytest.mark.parametrize(
    "factory",
    [
        create_remote_stack_git_source,
...
@pytest.mark.skipif(
    "KHULNASOFT_ACCESS_TOKEN" not in os.environ, reason="KHULNASOFT_ACCESS_TOKEN not set"
)
@pytest.mark.skipif(
    "KHULNASOFT_TEST_DEPLOYMENTS_API" not in os.environ,
    reason="KHULNASOFT_TEST_DEPLOYMENTS_API not set",
)
