...
instance as a gRPC server so that it can be used as a Khulnasoft plugin.
...
def main(provider: Provider, args: List[str]) -> None:  # args not in use?
    ...
    implementation into a Khulnasoft-compatible gRPC server.
    ...
    argp = argparse.ArgumentParser(description="Khulnasoft provider plugin (gRPC server)")
    argp.add_argument("engine", help="Khulnasoft engine address")
