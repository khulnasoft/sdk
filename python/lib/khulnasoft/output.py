...
class Output(Generic[T_co]):
    """
    Output helps encode the relationship between Resources in a Khulnasoft application. Specifically an
    ...
    def future(self, with_unknowns: Optional[bool] = None) -> Awaitable[Optional[T_co]]:
        ...
        # return None. This preserves compatibility with earlier versios of the Khulnasoft SDK.
    ...
    def __getattr__(self, item: str) -> "Output[Any]":  # type: ignore
        ...
        This is generally fine for most Khulnasoft use cases, but it can cause
...
See https://www.khulnasoft.com/docs/concepts/inputs-outputs for more details."""
        if os.getenv("KHULNASOFT_ERROR_OUTPUT_STRING", "").lower() in ["1", "true"]:
            raise TypeError(msg)
        msg += "\nThis function may throw in a future version of Khulnasoft."
        return msg
