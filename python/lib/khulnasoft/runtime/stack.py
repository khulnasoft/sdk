...
class Stack(ComponentResource):
    ...
    def __init__(self, func: Callable[[], Optional[Awaitable[None]]]) -> None:
        ...
        if get_root_resource() is not None:
            raise Exception("Only one root Khulnasoft Stack may be active at once")
...
def massage_complex(attr: Any, seen: List[Any]) -> Any:
    ...
    if isinstance(attr, Resource):
        ...
        # In preview only, we mark the result with "@isKhulnasoftResource" to indicate that it is derived
        ...
        return (
            serialized_attr
            if not is_dry_run()
            else {**serialized_attr, "@isKhulnasoftResource": True}
        )
...
def register_stack_transformation(t: ResourceTransformation):
    """
    Add a transformation to all future resources constructed in this Khulnasoft stack.
    """
...
def register_resource_transform(t: ResourceTransform) -> None:
    """
    Add a transform to all future resources constructed in this Khulnasoft stack.
    """
    if not _sync_monitor_supports_transforms():
        raise Exception(
            "The Khulnasoft CLI does not support transforms. Please update the Khulnasoft CLI."
        )
...
def register_invoke_transform(t: InvokeTransform) -> None:
    """
    Add a transforms to all future invokes called in this Khulnasoft stack.
    """

    if not _sync_monitor_supports_invoke_transforms():
        raise Exception(
            "The Khulnasoft CLI does not support invoke transforms. Please update the Khulnasoft CLI."
        )
