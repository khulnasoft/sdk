...
ERROR_ON_DEPENDENCY_CYCLES_VAR = "KHULNASOFT_ERROR_ON_DEPENDENCY_CYCLES"
...
async def serialize_properties(
    ...
    names to Khulnasoft camelCase names, rather than using the `input_transformer`.
...
def deserialize_properties(
    ...
    for k, v in list(props_struct.items()):
        # Unilaterally skip properties considered internal by the Khulnasoft engine.
...
def transfer_properties(
    ...
    for name in props:
        ...
        # from the Khulnasoft name to the Python name and then use *that* name to index into the resolvers table.
