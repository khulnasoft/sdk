...
from khulnasoft.automation._stack import Stack
...
def create_or_select_remote_stack_git_source(
    ...
    *,
    ...
    Git repository. Khulnasoft operations on the stack (Preview, Update, Refresh, and Destroy) are performed remotely.
    """
    if not _is_fully_qualified_stack_name(stack_name):
        raise Exception(f'stack name "{stack_name}" must be fully qualified.')
...
def _create_local_workspace(
    ...
    if not ws._version_check_opt_out():
        if not ws._remote_supported():
            raise Exception(
                "The Khulnasoft CLI version does not support remote operations. Please upgrade."
            )
        if inherit_settings and not ws._remote_inherit_settings_supported():
            raise Exception(
                "The Khulnasoft CLI version does not support remote inherit settings. Please upgrade."
            )
