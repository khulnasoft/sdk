...
from ._cmd import KhulnasoftCommand
...
KhulnasoftFn = Callable[[], Optional[Awaitable[None]]]
...
class WhoAmIResult:
    """The currently logged-in Khulnasoft identity."""
...
class Workspace(ABC):
    ...
    work_dir: str
    """
    The working directory to run Khulnasoft CLI commands
    """
    ...
    program: Optional[KhulnasoftFn]
    """
    The inline program `KhulnasoftFn` to be used for Preview/Update operations if any.
    ...
    khulnasoft_command: KhulnasoftCommand
    """
    The underlying KhulnasoftCommand instance that is used to execute CLI commands.
    """
    ...
    def list_stacks(self, include_all: Optional[bool] = None) -> List[StackSummary]:
        ...
        (as Khulnasoft.<stack>.yaml files).
