...
from ._workspace import Workspace, KhulnasoftFn, Deployment
...
class ImportResource(TypedDict, total=False):
    ...
    ImportResource represents a resource to import into a stack.
      ...
      - type: The type token of the Khulnasoft resource
    ...
    don't have an id set and instead just create an empty placeholder component resource in the Khulnasoft state.
    """
...
class Stack:
    ...
    def up(
        ...
        :param on_event: A function to process structured events from the Khulnasoft event stream.
    ...
    def _run_khulnasoft_cmd_sync(
        ...
        envs = {"KHULNASOFT_DEBUG_COMMANDS": "true"}
        if self._remote:
            envs = {**envs, "KHULNASOFT_EXPERIMENTAL": "true"}
        if self.workspace.khulnasoft_home is not None:
            envs = {**envs, "KHULNASOFT_HOME": self.workspace.khulnasoft_home}
