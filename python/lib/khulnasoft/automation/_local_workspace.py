...
from ._cmd import CommandResult, OnOutput, KhulnasoftCommand
...
class LocalWorkspaceOptions:
    ...
    program: Optional[KhulnasoftFn] = None
    ...
    khulnasoft_command: Optional[KhulnasoftCommand] = None
...
class LocalWorkspace(Workspace):
    ...
    def __init__(
        ...
        self.khulnasoft_command = khulnasoft_command or KhulnasoftCommand(
    ...
    def khulnasoft_version(self) -> str:  # type: ignore
        ...
        if v:
            return str(v)
        raise InvalidVersionError("Could not get Khulnasoft CLI version")
    ...
    def save_project_settings(self, settings: ProjectSettings) -> None:
        ...
        for ext in _setting_extensions:
            test_path = os.path.join(self.work_dir, f"Khulnasoft{ext}")
            if os.path.exists(test_path):
                found_ext = ext
                break
        path = os.path.join(self.work_dir, f"Khulnasoft{found_ext}")
    ...
    def stack_settings(self, stack_name: str) -> StackSettings:
        ...
        for ext in _setting_extensions:
            path = os.path.join(self.work_dir, f"Khulnasoft.{stack_settings_name}{ext}")
    ...
    def save_stack_settings(self, stack_name: str, settings: StackSettings) -> None:
        ...
        for ext in _setting_extensions:
            test_path = os.path.join(
                self.work_dir, f"Khulnasoft.{stack_settings_name}{ext}"
            )
            if os.path.exists(test_path):
                found_ext = ext
                break
        path = os.path.join(self.work_dir, f"Khulnasoft.{stack_settings_name}{found_ext}")
        with open(path, "w", encoding="utf-8") as file:
            if found_ext == ".json":
    ...
    def _run_khulnasoft_cmd_sync(
        ...
        envs = {"KHULNASOFT_HOME": self.khulnasoft_home} if self.khulnasoft_home else {}
        if self._remote:
...
def create_stack(
    ...
    """
    Creates a Stack with a LocalWorkspace utilizing the specified inline (in process) Khulnasoft program or the local
    Khulnasoft CLI program from the specified working dir.
    ...
    **Local Programs**
    ...
    This is a way to create drivers on top of pre-existing Khulnasoft programs. This Workspace will pick up any
    available Settings files (Khulnasoft.yaml, Khulnasoft.[stack].yaml).
...
def select_stack(
    ...
    """
    Selects a Stack with a LocalWorkspace utilizing the specified inline (in process) Khulnasoft program or the local
    Khulnasoft CLI program from the specified working dir.
...
def create_or_select_stack(
    ...
    """
    Creates or selects an existing Stack with a LocalWorkspace utilizing the specified inline (in process) Khulnasoft
    program or the local Khulnasoft CLI program from the specified working dir.
...
def _load_project_settings(work_dir: str) -> ProjectSettings:
    for ext in _setting_extensions:
        project_path = os.path.join(work_dir, f"Khulnasoft{ext}")
