...
class KhulnasoftCommand:
    """
    KhulnasoftCommand manages the Khulnasoft CLI. It can be used to to install the CLI and run commands.
    """
    ...
    def __init__(
        ...
        """
        Creates a new KhulnasoftCommand.

        :param root: The directory where to look for the Khulnasoft installation. Defaults to running `khulnasoft` from $PATH.
        :param version: The minimum version of the Khulnasoft CLI to use and validates that it is compatbile with this version.
        :param skip_version_check: If true the version validation will be skipped. The env variable
                `KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK` also disable this check, and takes precendence. If it is set it
    ...
    def install(
        ...
        """
        Downloads and installs the Khulnasoft CLI.  By default the CLI version
    ...
    def run(
        ...
        """
        Runs a Khulnasoft command, returning a CommandResult. If the command fails, a CommandError is raised.

        :param args: The arguments to pass to the Khulnasoft CLI, for example `["stack", "ls"]`.
    ...
    *None* is returned.
    ...
    if version is None:
        raise InvalidVersionError(
            f"Could not parse the Khulnasoft CLI version. This is probably an internal error. "
            f"If you are sure you have the correct version, set {_SKIP_VERSION_CHECK_VAR}=true."
        )
    if min_version.major < version.major:
        raise InvalidVersionError(
            f"Major version mismatch. You are using Khulnasoft CLI version {version} with "
    ...
    if min_version.compare(version) == 1:
        raise InvalidVersionError(
            ...
            f"Please update the Khulnasoft CLI."
        )
    return version
