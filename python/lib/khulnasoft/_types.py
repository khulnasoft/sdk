...
# `khulnasoft.property()` to indicate the Khulnasoft property name.
#
...
# specify the types and Khulnasoft input property names. An `__init__()` method is automatically added
...
# `khulnasoft.get` and `khulnasoft.set` will call to translate the Khulnasoft property name to a translated
...
_KHULNASOFT_NAME = "_khulnasoft_name"
_KHULNASOFT_INPUT_TYPE = "_khulnasoft_input_type"
_KHULNASOFT_OUTPUT_TYPE = "_khulnasoft_output_type"
_KHULNASOFT_PYTHON_TO_KHULNASOFT_TABLE = "_khulnasoft_python_to_khulnasoft_table"
_KHULNASOFT_DEPRECATED_CALLABLE = "_khulnasoft_deprecated_callable"
_TRANSLATE_PROPERTY = "_translate_property"


def is_input_type(cls: type) -> bool:
    return hasattr(cls, _KHULNASOFT_INPUT_TYPE)


def is_output_type(cls: type) -> bool:
    return hasattr(cls, _KHULNASOFT_OUTPUT_TYPE)
...
class _Property:
    """
    Represents a Khulnasoft property. It is not meant to be created outside this module,
...
def property(name: str, *, default: Any = MISSING) -> Any:  # noqa: A001 shadowing builtin
    """
    Return an object to identify Khulnasoft properties.

    name is the Khulnasoft property name.
    """
...
def _create_py_property(a_name: str, khulnasoft_name: str, typ: Any, setter: bool = False):
    ...
    setattr(getter_fn, _KHULNASOFT_NAME, khulnasoft_name)
    ...
    for base in reversed(cls.__mro__):
        for python_name, v in base.__dict__.items():
            if isinstance(v, builtins.property):
                ...
                khulnasoft_name = getattr(prop.fget, _KHULNASOFT_NAME, MISSING)
...
def input_type(cls: Type[T]) -> Type[T]:
    ...
    _process_class(cls, _KHULNASOFT_INPUT_TYPE, is_input=True, setter=True)
...
def input_type_py_to_khulnasoft_names(input_type_cls: Type) -> Dict[str, str]:
    """
    Returns a dict of Python names to Khulnasoft names for the input type.
    """
...
def input_type_types(input_type_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Khulnasoft names to types for the input type.
    """
    assert is_input_type(input_type_cls)
...
def input_type_to_dict(obj: Any) -> Dict[str, Any]:
    ...
    The keys of the dict are Khulnasoft names that should not be translated.
    """
    cls = type(obj)
    ...
    for _, khulnasoft_name, prop in _py_properties(cls):
        ...
        deprecated_callable = prop.fget.__dict__.get(_KHULNASOFT_DEPRECATED_CALLABLE)
...
def output_type(cls: Type[T]) -> Type[T]:
    ...
    Python property getters are created for each Khulnasoft output property
    ...
    _process_class(cls, _KHULNASOFT_OUTPUT_TYPE)

    # If the class has a _translate_property() method, build a mapping table of Python names to
    # Khulnasoft names. Calls to khulnasoft.get() will then convert the name passed to khulnasoft.get() from
    # the Python name to the Khulnasoft name, and then pass the Khulnasoft name to _translate_property() to
    # convert the Khulnasoft name to whatever name _translate_property() returns (which, for our
    ...
    if hasattr(cls, _TRANSLATE_PROPERTY):
        ...
        if python_to_khulnasoft_table is not None:
            setattr(cls, _KHULNASOFT_PYTHON_TO_KHULNASOFT_TABLE, python_to_khulnasoft_table)
...
def getter(_fn=None, *, name: Optional[str] = None):
    """
    Decorator to indicate a function is a Khulnasoft property getter.

    name is the Khulnasoft property name. If not set, the name of the function is used.
    """
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        if _utils.is_empty_function(fn):
            ...
            fn = get_fn
        setattr(fn, _KHULNASOFT_NAME, khulnasoft_name)
        return fn
...
def _translate_name(obj: Any, name: str) -> str:
    cls = type(obj)
    if hasattr(cls, _KHULNASOFT_OUTPUT_TYPE):
        # If the class has a _translate_property() method we need to do two translations:
        #   1. Translate Python => Khulnasoft name.
        #   2. Translate Khulnasoft name => result of _translate_property().
        translate = getattr(cls, _TRANSLATE_PROPERTY, None)
        if callable(translate):
            table = getattr(cls, _KHULNASOFT_PYTHON_TO_KHULNASOFT_TABLE, None)
...
def get(self, name: str) -> Any:
    ...
    if hasattr(cls, _KHULNASOFT_INPUT_TYPE) or hasattr(cls, _KHULNASOFT_OUTPUT_TYPE):
        if hasattr(cls, _KHULNASOFT_OUTPUT_TYPE):
...
def _types_from_py_properties(cls: type) -> Dict[str, type]:
    """
    Returns a dict of Khulnasoft names to types for a type.
    """
    from . import Output
    ...
    # Build-up a dictionary of Khulnasoft property names to types by looping through all the
    # Python properties on the class that have a getter marked as a Khulnasoft property getter,
...
def _types_from_annotations(cls: type) -> Dict[str, type]:
    ...
    Returns a dict of Khulnasoft names to types for a type.
    """
    # Get the "Khulnasoft properties" from the class's type annotations.
    props = _properties_from_annotations(cls)
    if not props:
        return {}
    ...
    # Return a dictionary of Khulnasoft property names to types. Types that are Output[T] and
...
def output_type_types(output_type_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Khulnasoft names to types for the output type.
    """
...
def resource_types(resource_cls: type) -> Dict[str, type]:
    """
    Returns a dict of Khulnasoft names to types for the resource.
    """
