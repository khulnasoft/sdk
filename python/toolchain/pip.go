...
type pip struct {
	...
	// The virtual option as set in Khulnasoft.yaml.
...
func (p *pip) EnsureVenv(ctx context.Context, cwd string, useLanguageVersionTools,
	...
	if err != nil {
		...
		return fmt.Errorf("the 'virtualenv' option in Khulnasoft.yaml is set to %q but it is not a directory", p.virtualenvPath)
	}
...
// CommandPath finds the correct path and command for Python. If the `KHULNASOFT_PYTHON_CMD`
...
func CommandPath() (string /*pythonPath*/, string /*pythonCmd*/, error) {
	...
	if pythonCmd := os.Getenv("KHULNASOFT_PYTHON_CMD"); pythonCmd != "" {
...
func NewVirtualEnvError(dir, fullPath string) error {
	...
	return fmt.Errorf("The 'virtualenv' option in Khulnasoft.yaml is set to %q, but %q %s; "+
