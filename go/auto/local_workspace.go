...
import (
	...
	"github.com/khulnasoft/sdk/go/auto/optlist"
	"github.com/khulnasoft/sdk/go/auto/optremove"
...
// A Workspace is the execution context containing a single Khulnasoft project, a program,
...
// ($KHULNASOFT_HOME), and creation, deletion, and listing of Stacks.
// LocalWorkspace relies on Khulnasoft.yaml and Khulnasoft.<stack>.yaml as the intermediate format
...
// This is identical to the behavior of Khulnasoft CLI driven workspaces.
...
// LocalWorkspace reads settings from the Khulnasoft.yaml in the workspace.
...
// LocalWorkspace writes this value to a Khulnasoft.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) SaveProjectSettings(ctx context.Context, settings *workspace.Project) error {
	khulnasoftYamlPath := filepath.Join(l.WorkDir(), "Khulnasoft.yaml")
...
// LocalWorkspace reads this from a Khulnasoft.<stack>.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) StackSettings(ctx context.Context, stackName string) (*workspace.ProjectStack, error) {
	...
	for _, ext := range settingsExtensions {
		stackPath := filepath.Join(l.WorkDir(), fmt.Sprintf("Khulnasoft.%s%s", name, ext))
...
// LocalWorkspace writes this value to a Khulnasoft.<stack>.yaml file in Workspace.WorkDir()
func (l *LocalWorkspace) SaveStackSettings(
	...
	name := getStackSettingsName(stackName)
	stackYamlPath := filepath.Join(l.WorkDir(), fmt.Sprintf("Khulnasoft.%s.yaml", name))
...
// An extensibility point to perform workspace cleanup (CLI operations may create/modify a Khulnasoft.stack.yaml)
// LocalWorkspace does not utilize this extensibility point.
...
func (l *LocalWorkspace) AddEnvironments(ctx context.Context, stackName string, envs ...string) error {
	...
	if l.khulnasoftCommand.Version().LT(semver.Version{Major: 3, Minor: 95}) {
		return errors.New("AddEnvironments requires Khulnasoft CLI version >= 3.95.0")
	}
	args := []string{"config", "env", "add"}
	args = append(args, envs...)
	args = append(args, "--yes", "--stack", stackName)
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, args...)
	if err != nil {
...
func (l *LocalWorkspace) ListEnvironments(ctx context.Context, stackName string) ([]string, error) {
	...
	if l.khulnasoftCommand.Version().LT(semver.Version{Major: 3, Minor: 99}) {
		return nil, errors.New("ListEnvironments requires Khulnasoft CLI version >= 3.99.0")
	}
...
func (l *LocalWorkspace) RemoveEnvironment(ctx context.Context, stackName string, env string) error {
	// 3.95 added this command (https://github.com/khulnasoft/khulnasoft/releases/tag/v3.95.0)
	if l.khulnasoftCommand.Version().LT(semver.Version{Major: 3, Minor: 95}) {
		return errors.New("RemoveEnvironments requires Khulnasoft CLI version >= 3.95.0")
	}
...
// scoped to the current workspace. LocalWorkspace reads this config from the matching Khulnasoft.stack.yaml file.
...
// LocalWorkspace reads this config from the matching Khulnasoft.stack.yaml file.
...
func (l *LocalWorkspace) GetAllConfig(ctx context.Context, stackName string) (ConfigMap, error) {
	var val ConfigMap
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "config", "--show-secrets", "--json", "--stack", stackName)
	if err != nil {
		return val, newAutoError(fmt.Errorf("unable to read config: %w", err), stdout, stderr, errCode)
	}
	err = json.Unmarshal([]byte(stdout), &val)
...
// LocalWorkspace writes this value to the matching Khulnasoft.<stack>.yaml file in Workspace.WorkDir().
...
// LocalWorkspace writes the config to the matching Khulnasoft.<stack>.yaml file in Workspace.WorkDir().
...
// It will remove any matching values in the Khulnasoft.<stack>.yaml file in Workspace.WorkDir().
...
// It will overwrite all configuration in the Khulnasoft.<stack>.yaml file in Workspace.WorkDir().
func (l *LocalWorkspace) RefreshConfig(ctx context.Context, stackName string) (ConfigMap, error) {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "config", "refresh", "--force", "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) GetTag(ctx context.Context, stackName string, key string) (string, error) {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "tag", "get", key, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) SetTag(ctx context.Context, stackName string, key string, value string) error {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "tag", "set", key, value, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) RemoveTag(ctx context.Context, stackName string, key string) error {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "tag", "rm", key, "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) ListTags(ctx context.Context, stackName string) (map[string]string, error) {
	...
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "tag", "ls", "--json", "--stack", stackName)
	if err != nil {
...
// WorkDir returns the working directory to run Khulnasoft CLI commands.
// LocalWorkspace expects that this directory contains a Khulnasoft.yaml file.
// For "Inline" Khulnasoft programs created from NewStackInlineSource, a Khulnasoft.yaml
// is created on behalf of the user if none is specified.
func (l *LocalWorkspace) WorkDir() string {
	return l.workDir
}

// KhulnasoftCommand returns the KhulnasoftCommand instance that is used to execute commands.
func (l *LocalWorkspace) KhulnasoftCommand() KhulnasoftCommand {
	return l.khulnasoftCommand
}

// KhulnasoftHome returns the directory override for CLI metadata if set.
// This customizes the location of $KHULNASOFT_HOME where metadata is stored and plugins are installed.
func (l *LocalWorkspace) KhulnasoftHome() string {
	return l.khulnasoftHome
}

// KhulnasoftVersion returns the version of the underlying Khulnasoft CLI/Engine.
func (l *LocalWorkspace) KhulnasoftVersion() string {
...
func (l *LocalWorkspace) WhoAmI(ctx context.Context) (string, error) {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "whoami")
	if err != nil {
...
func (l *LocalWorkspace) WhoAmIDetails(ctx context.Context) (WhoAmIResult, error) {
	...
	if l.khulnasoftCommand.Version().GTE(semver.Version{Major: 3, Minor: 58}) {
		...
		stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "whoami", "--json")
		if err != nil {
...
func (l *LocalWorkspace) ChangeStackSecretsProvider(
	...
	stdout, stderr, errCode, err := l.runKhulnasoftInputCmdSync(ctx, reader, nil, nil, args...)
	if err != nil {
...
// This queries underlying backend and may return stacks not present in the Workspace (as Khulnasoft.<stack>.yaml files).
...
func (l *LocalWorkspace) InstallPlugin(ctx context.Context, name string, version string) error {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "plugin", "install", "resource", name, version)
	if err != nil {
...
func (l *LocalWorkspace) InstallPluginFromServer(
	...
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(
...
func (l *LocalWorkspace) RemovePlugin(ctx context.Context, name string, version string) error {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "plugin", "rm", "resource", name, version, "--yes")
	if err != nil {
...
func (l *LocalWorkspace) ListPlugins(ctx context.Context) ([]workspace.PluginInfo, error) {
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "plugin", "ls", "--json")
	if err != nil {
...
func (l *LocalWorkspace) ExportStack(ctx context.Context, stackName string) (apitype.UntypedDeployment, error) {
	...
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "export", "--show-secrets", "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) ImportStack(ctx context.Context, stackName string, state apitype.UntypedDeployment) error {
	...
	stdout, stderr, errCode, err := l.runKhulnasoftCmdSync(ctx, "stack", "import", "--file", f.Name(), "--stack", stackName)
	if err != nil {
...
func (l *LocalWorkspace) StackOutputs(ctx context.Context, stackName string) (OutputMap, error) {
	// standard outputs
	outStdout, outStderr, code, err := l.runKhulnasoftCmdSync(ctx, "stack", "output", "--json", "--stack", stackName)
	if err != nil {
		return nil, newAutoError(fmt.Errorf("could not get outputs: %w", err), outStdout, outStderr, code)
	}

	// secret outputs
	secretStdout, secretStderr, code, err := l.runKhulnasoftCmdSync(ctx,
...
func (l *LocalWorkspace) Install(ctx context.Context, opts *InstallOptions) error {
	...
	if opts != nil && opts.UseLanguageVersionTools {
		// Khulnasoft 3.130.0 introduced the `--use-language-version-tools` flag.
		if l.khulnasoftCommand.Version().LT(semver.Version{Major: 3, Minor: 130}) {
			return errors.New("UseLanguageVersionTools requires Khulnasoft CLI version >= 3.130.0")
		}
	...
	stdout, stderr, errCode, err := l.runKhulnasoftInputCmdSync(ctx, nil, stdoutWriters, stderrWriters, args...)
	if err != nil {
...
func (l *LocalWorkspace) runKhulnasoftInputCmdSync(
	ctx context.Context,
	stdin io.Reader,
	...
	var env []string
	if l.KhulnasoftHome() != "" {
		homeEnv := fmt.Sprintf("%s=%s", khulnasoftHomeEnv, l.KhulnasoftHome())
	...
	return l.KhulnasoftCommand().Run(ctx,
		l.WorkDir(),
		stdin,
		additionalOutputs,
...
func (l *LocalWorkspace) runKhulnasoftCmdSync(
	ctx context.Context,
	args ...string,
) (string, string, int, error) {
	return l.runKhulnasoftInputCmdSync(ctx, nil, nil, nil, args...)
}

// supportsKhulnasoftCmdFlag runs a command with `--help` to see if the specified flag is found within the resulting
// output, in which case we assume the flag is supported.
func (l *LocalWorkspace) supportsKhulnasoftCmdFlag(ctx context.Context, flag string, args ...string) (bool, error) {
	env := []string{
		"KHULNASOFT_DEBUG_COMMANDS=true",
		"KHULNASOFT_EXPERIMENTAL=true",
	}

	// Run the command with `--help`, and then we'll look for the flag in the output.
	stdout, _, _, err := l.KhulnasoftCommand().Run(ctx, l.WorkDir(), nil, nil, nil, env, append(args, "--help")...)
	if err != nil {
		return false, err
	}
...
func NewLocalWorkspace(ctx context.Context, opts ...LocalWorkspaceOption) (Workspace, error) {
	...
	var khulnasoftCommand KhulnasoftCommand
	if lwOpts.KhulnasoftCommand != nil {
		khulnasoftCommand = lwOpts.KhulnasoftCommand
	} else {
		p, err := NewKhulnasoftCommand(&KhulnasoftCommandOptions{SkipVersionCheck: optOut})
		if err != nil {
			return nil, err
		}
		khulnasoftCommand = p
	...
	l := &LocalWorkspace{
		...
		program:                       program,
		khulnasoftHome:                lwOpts.KhulnasoftHome,
	...
	if !optOut && l.remote {
		...
		supportsRemote, err := l.supportsKhulnasoftCmdFlag(ctx, "--remote", "preview")
		if err != nil {
			return nil, err
		}
		if !supportsRemote {
			return nil, errors.New("Khulnasoft CLI does not support remote operations; please upgrade")
		}
	}
...
type localWorkspaceOptions struct {
	...
	WorkDir string
	// Program is the Khulnasoft Program to execute. If none is supplied,
	// the program identified in $WORKDIR/Khulnasoft.yaml will be used instead.
	Program khulnasoft.RunFunc
	// KhulnasoftHome overrides the metadata directory for khulnasoft commands.
	// This customizes the location of $KHULNASOFT_HOME where metadata is stored and plugins are installed.
	KhulnasoftHome string
	// KhulnasoftCommand is the KhulnasoftCommand instance to use. If none is
	// supplied, the workspace will create an instance using the KhulnasoftCommand
	// CLI found in $PATH.
	KhulnasoftCommand KhulnasoftCommand
	...
	// Repo is a git repo with a Khulnasoft Project to clone into the WorkDir.
	Repo *GitRepo
	...
	Remote bool
	// Remote environment variables to be passed to the remote Khulnasoft operation.
	RemoteEnvVars map[string]EnvVarValue
	// PreRunCommands is an optional list of arbitrary commands to run before the remote Khulnasoft operation is invoked.
	PreRunCommands []string
	// RemoteSkipInstallDependencies sets whether to skip the default dependency installation step
	RemoteSkipInstallDependencies bool
	// RemoteExecutorImage is the image to use for the remote Khulnasoft operation.
	RemoteExecutorImage *ExecutorImage
	// RemoteAgentPoolID is the agent pool (also called deployment runner pool) to use for the remote Khulnasoft operation.
	RemoteAgentPoolID string
...
// See Workdir, Program, KhulnasoftHome, Project, Stacks, and Repo for concrete options.
...
// GitRepo contains info to acquire and setup a Khulnasoft program from a git repository.
type GitRepo struct {
...
// KhulnasoftHome overrides the metadata directory for khulnasoft commands.
func KhulnasoftHome(dir string) LocalWorkspaceOption {
	return localWorkspaceOption(func(lo *localWorkspaceOptions) {
		lo.KhulnasoftHome = dir
	})
}
...
// supplied, the workspace will create an instance using the KhulnasoftCommand
// CLI found in $PATH.
func Khulnasoft(khulnasoft KhulnasoftCommand) LocalWorkspaceOption {
	return localWorkspaceOption(func(lo *localWorkspaceOptions) {
		lo.KhulnasoftCommand = khulnasoft
	})
}
...
// These values will be passed to the remote Khulnasoft operation.
...
// any available Settings files (Khulnasoft.yaml, Khulnasoft.<stack>.yaml).
...
// Settings files (Khulnasoft.yaml, Khulnasoft.<stack>.yaml).
...
// any available Settings files (Khulnasoft.yaml, Khulnasoft.<stack>.yaml) that are cloned into the Workspace.
...
// any available Settings files (Khulnasoft.yaml, Khulnasoft.<stack>.yaml) that are cloned
