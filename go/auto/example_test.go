...
func ExampleLocalWorkspace_GetAllConfig() {
	...
	// create a workspace from a local project
	w, _ := NewLocalWorkspace(ctx, WorkDir(filepath.Join(".", "program")))
	// get all config from the workspace for the stack (read from Khulnasoft.stack.yaml)
...
func ExampleNewLocalWorkspace() {
	...
	// project settings file (Khulnasoft.yaml) in this location for information about the Khulnasoft program.
	wd := WorkDir(filepath.Join("..", "path", "to", "khulnasoft", "project"))
	// KhulnasoftHome customizes the location of $KHULNASOFT_HOME where metadata is stored and plugins are installed.
	ph := KhulnasoftHome(filepath.Join("~", ".khulnasoft"))
...
func ExampleLocalWorkspace_ProjectSettings() {
	...
	if err != nil {
		// no Khulnasoft.yaml was found, so create a default
...
func ExampleStack_Info() {
	...
	// url to view the Stack in the Khulnasoft SaaS or other backend
...
func ExampleStack_RefreshConfig() {
	...
	stackName := FullyQualifiedStackName("org", "project", "stack")
	stack, _ := SelectStackLocalSource(ctx, stackName, filepath.Join(".", "program"))
	// get the last deployed config from stack and overwrite Khulnasoft.stack.yaml
