...
import (
	...
	resourceConfig "github.com/khulnasoft/sdk/go/common/resource/config"
...
type mockKhulnasoftCommand struct {
...
func (m *mockKhulnasoftCommand) Version() semver.Version {
	return m.version
}

func (m *mockKhulnasoftCommand) Run(ctx context.Context,
	workdir string,
	stdin io.Reader,
...
func TestWorkspaceSecretsProvider(t *testing.T) {
	...
	mkstack := func(passphrase string) Stack {
		opts := []LocalWorkspaceOption{
			...
			EnvVars(map[string]string{
				"KHULNASOFT_CONFIG_PASSPHRASE": passphrase,
			}),
		}
	...
	defer func() {
		err := os.Unsetenv("KHULNASOFT_CONFIG_PASSPHRASE")
...
// This test requires the existence of a Khulnasoft.dev.yaml file because we are reading the nested
...
func TestNestedConfig(t *testing.T) {
	...
	// make sure the config agrees with the config loaded by Khulnasoft.
...
func TestKhulnasoftVersion(t *testing.T) {
	t.Parallel()
	...
	version := ws.KhulnasoftVersion()
	assert.NotEqual(t, "v0.0.0", version)
	assert.Regexp(t, `(\d+\.)(\d+\.)(\d+)(-.*)?`, version)
}

func TestKhulnasoftCommand(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	khulnasoftCommand, err := NewKhulnasoftCommand(nil)
	require.NoError(t, err, "failed to create khulnasoft command: %s", err)
	ws, err := NewLocalWorkspace(ctx, Khulnasoft(khulnasoftCommand))
...
func TestClIWithoutRemoteSupport(t *testing.T) {
	...
	m := mockKhulnasoftCommand{stdout: "some output"}

	_, err := NewLocalWorkspace(context.Background(), Khulnasoft(&m), remote(true))
...
func TestByPassesRemoteCheck(t *testing.T) {
	...
	// simulate a CLI version without remote support.
	m := mockKhulnasoftCommand{stdout: "some output"}
	envVars := map[string]string{"KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK": "true"}

	_, err := NewLocalWorkspace(context.Background(), Khulnasoft(&m), EnvVars(envVars), remote(true))
...
func TestListStacks(t *testing.T) {
	...
	workspace, err := NewLocalWorkspace(ctx, WorkDir(pDir), Khulnasoft(&m))
	require.NoError(t, err)
...
func TestInstallWithUseLanguageVersionTools(t *testing.T) {
	...
	require.ErrorContains(t, err, "UseLanguageVersionTools requires Khulnasoft CLI version >= 3.130.0")
...
func getTestOrg() string {
	...
	if _, set := os.LookupEnv("KHULNASOFT_TEST_ORG"); set {
		testOrg = os.Getenv("KHULNASOFT_TEST_ORG")
	}
	return testOrg
}
