...
import (
	...
	"github.com/khulnasoft/sdk/go/auto/optremotepreview"
...
func testRemoteStackGitSource(
	...
	// This test requires the service with access to Khulnasoft Deployments.
	// Set KHULNASOFT_ACCESS_TOKEN to an access token with access to Khulnasoft Deployments
	// and set KHULNASOFT_TEST_DEPLOYMENTS_API to any value to enable the test.
	if os.Getenv("KHULNASOFT_ACCESS_TOKEN") == "" {
		t.Skipf("Skipping: KHULNASOFT_ACCESS_TOKEN is not set")
	}
	if os.Getenv("KHULNASOFT_TEST_DEPLOYMENTS_API") == "" {
		t.Skipf("Skipping: KHULNASOFT_TEST_DEPLOYMENTS_API is not set")
	}
