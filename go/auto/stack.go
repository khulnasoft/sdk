...
import (
	...
	"github.com/khulnasoft/sdk/go/auto/optimport"
	...
	"github.com/khulnasoft/sdk/go/auto/opthistory"
...
// Stack is an isolated, independently configurable instance of a Khulnasoft program.
...
func (s *Stack) Preview(ctx context.Context, opts ...optpreview.Option) (PreviewResult, error) {
	...
	stdout, stderr, code, err := s.runKhulnasoftCmdSync(
		ctx,
...
func (s *Stack) Up(ctx context.Context, opts ...optup.Option) (UpResult, error) {
	...
	args = append(args, sharedArgs...)
	stdout, stderr, code, err := s.runKhulnasoftCmdSync(ctx, upOpts.ProgressStreams, upOpts.ErrorProgressStreams, args...)
	if err != nil {
...
func (s *Stack) PreviewRefresh(ctx context.Context, opts ...optrefresh.Option) (PreviewResult, error) {
	...
	if s.Workspace().KhulnasoftCommand().Version().LT(semver.Version{Major: 3, Minor: 105}) {
		return res, errors.New("PreviewRefresh requires Khulnasoft CLI version >= 3.105.0")
	}
...
func (s *Stack) PreviewDestroy(ctx context.Context, opts ...optdestroy.Option) (PreviewResult, error) {
	...
	// 3.105.0 added this flag (https://github.com/khulnasoft/khulnasoft/releases/tag/v3.105.0)
	if minVer := (semver.Version{Major: 3, Minor: 105}); s.Workspace().KhulnasoftCommand().Version().LT(minVer) {
		return res, fmt.Errorf("PreviewRefresh requires Khulnasoft CLI version >= %s", minVer)
	}
...
func (s *Stack) History(ctx context.Context,
	...
	stdout, stderr, errCode, err := s.runKhulnasoftCmdSync(
		ctx,
...
// OutputValue models a Khulnasoft Stack output, providing the plaintext value and a boolean indicating secretness.
...
// GetPermalink returns the permalink URL in the Khulnasoft Console for the update operation.
...
// GetPermalink returns the permalink URL in the Khulnasoft Console for the update
...
// OutputMap is the output result of running a Khulnasoft program
...
// GetPermalink returns the permalink URL in the Khulnasoft Console for the preview operation.
...
// GetPermalink returns the permalink URL in the Khulnasoft Console for the refresh operation.
...
// GetPermalink returns the permalink URL in the Khulnasoft Console for the destroy operation.
...
func (s *Stack) runKhulnasoftCmdSync(
	ctx context.Context,
	...
	var env []string
	debugEnv := fmt.Sprintf("%s=%s", "KHULNASOFT_DEBUG_COMMANDS", "true")
	...
	if remote {
		experimentalEnv := fmt.Sprintf("%s=%s", "KHULNASOFT_EXPERIMENTAL", "true")
		env = append(env, experimentalEnv)
	}

	if s.Workspace().KhulnasoftHome() != "" {
		homeEnv := fmt.Sprintf("%s=%s", khulnasoftHomeEnv, s.Workspace().KhulnasoftHome())
	...
	stdout, stderr, errCode, err := s.workspace.KhulnasoftCommand().Run(
		ctx,
		s.Workspace().WorkDir(),
		nil,
		additionalOutput,
...
func isNestedInvocation() bool {
	...
	for {
		...
		for f, more := frames.Next(); more; f, more = frames.Next() {
			if f.Function == "github.com/khulnasoft/sdk/go/khulnasoft.RunWithContext" {
				return true
			}
		}
	}
