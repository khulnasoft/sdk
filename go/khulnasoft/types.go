...
// Output helps encode the relationship between resources in a Khulnasoft application. Specifically an output property
...
// RegisterOutputType registers an Output type with the Khulnasoft runtime. If a value of this type's concrete type is
...
// RegisterInputType registers an Input type with the Khulnasoft runtime. This allows the input type to be instantiated
...
// For example:
//
//	connectionString := khulnasoft.All(sqlServer.Name, database.Name).ApplyT(
//		func (args []interface{}) khulnasoft.Output {
//			return Connection{
...
// For example:
//
//	connectionString := khulnasoft.AllWithContext(ctx.Context(), sqlServer.Name, database.Name).ApplyT(
//		func (args []interface{}) khulnasoft.Output {
//			return Connection{
//				Server: args[0].(string),
//				Database: args[1].(string),
...
// a top level unknown with [khulnasoft.Input.ApplyT] or [All]. This does not work:
//
//	khulnasoft.JSONMarshal(map[string]any{"key": myResource.Name})
//
// You need to move the output myResource.Name to a top level output:
//
//	khulnasoft.JSONMarshal(myResource.Name.Apply(func(name string) map[string]any{
//		return map[string]any{"key": name}
//	}))
//
// Supporting nested unknowns is tracked in https://github.com/khulnasoft/khulnasoft/issues/12460
...
// manually create a top level unknown with [khulnasoft.Input.ApplyT] or [All]. This does not work:
//
//	khulnasoft.JSONMarshalWithContext(ctx.Context(), map[string]any{"key": myResource.Name})
//
// You need to move the output myResource.Name to a top level output:
//
//	khulnasoft.JSONMarshalWithContext(ctx.Context(), myResource.Name.Apply(func(name string) map[string]any{
//		return map[string]any{"key": name}
//	}))
//
// Supporting nested unknowns is tracked in https://github.com/khulnasoft/khulnasoft/issues/12460
...
// Input is the type of a generic input value for a Khulnasoft resource. This type is used in conjunction with Output
// to provide polymorphism over strongly-typed input values.
//
// The intended pattern for nested Khulnasoft value types is to define an input interface and a plain, input, and output
// variant of the value type that implement the input interface.
//
// For example, given a nested Khulnasoft value type with the following shape:
//
//	type Nested struct {
//	    Foo int
//	    Bar string
