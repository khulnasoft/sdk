...
type KhulnasoftParameterizationJSON struct {
...
// KhulnasoftPluginJSON represents additional information about a package's associated Khulnasoft plugin.
...
type KhulnasoftPluginJSON struct {
	...
	Parameterization *KhulnasoftParameterizationJSON `json:"parameterization,omitempty"`
}

func (plugin *KhulnasoftPluginJSON) JSON() ([]byte, error) {
...
func LoadKhulnasoftPluginJSON(path string) (*KhulnasoftPluginJSON, error) {
...
// the stack's Khulnasoft SDK did not have the required modules. i.e. is too old.
...
func execPlugin(ctx *Context, bin, prefix string, kind apitype.PluginKind,
	...
	if _, err := os.Stat(bin); os.IsNotExist(err) {
		// If we don't have the expected binary, see if we have a "KhulnasoftPlugin.yaml" or "KhulnasoftPolicy.yaml"
		...
		if kind == apitype.ResourcePlugin || kind == apitype.ConverterPlugin {
			proj, err := workspace.LoadPluginProject(filepath.Join(pluginDir, "KhulnasoftPlugin.yaml"))
			if err != nil {
				return nil, fmt.Errorf("loading KhulnasoftPlugin.yaml: %w", err)
			}
			runtimeInfo = proj.Runtime
		} else if kind == apitype.AnalyzerPlugin {
			proj, err := workspace.LoadPluginProject(filepath.Join(pluginDir, "KhulnasoftPolicy.yaml"))
			if err != nil {
				return nil, fmt.Errorf("loading KhulnasoftPolicy.yaml: %w", err)
			}
			runtimeInfo = proj.Runtime
		} else {
