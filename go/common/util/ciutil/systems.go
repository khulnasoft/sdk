...
type baseCI struct {
	...
	// which would indicate that the Khulnasoft CLI is running in that CI system's environment.
	EnvVarsToDetect []string
	// EnvValuesToDetect is a map of env vars and their expected values to check for,
	// in order to see if the Khulnasoft CLI is running inside a certain CI system's environment.
