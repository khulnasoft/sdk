...
func newGitIgnoreIgnorer(pathToKhulnasoftIgnore string) (ignorer, error) {
	gitIgnorer, err := ignore.CompileIgnoreFile(pathToKhulnasoftIgnore)
	if err != nil {
		return nil, err
	}

	return &gitIngoreIgnorer{root: path.Dir(pathToKhulnasoftIgnore), ignorer: gitIgnorer}, nil
}
