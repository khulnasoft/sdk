...
// and invocation of the Khulnasoft engine that caused it.
type SnapshotIntegrityErrorMetadataV1 struct {
	// The version of the Khulnasoft engine that caused the integrity error.
	Version string `json:"version,omitempty" yaml:"version,omitempty"`
	// The command/invocation of the Khulnasoft engine that caused the integrity error.
...
type UntypedDeployment struct {
	...
	// The opaque Khulnasoft deployment. This is conceptually of type `Deployment`, but we use `json.Message` to
...
// ResourceV1 describes a Cloud resource constructed by Khulnasoft.
...
//     resources that Khulnasoft does not own in the deployment.
...
//     implies that it is owned by Khulnasoft. Note that since this is the default value for
...
type ResourceV2 struct {
	...
	// Protect is set to true when this resource is "protected" and may not be deleted.
	Protect bool `json:"protect,omitempty" yaml:"protect,omitempty"`
	// External is set to true when the lifecycle of this resource is not managed by Khulnasoft.
...
type ResourceV3 struct {
	...
	// If set to True, the providers Delete method will not be called for this resource. Khulnasoft simply stops tracking the deleted resource.
...
type ManifestV1 struct {
	...
	// Version of the Khulnasoft engine used to render the checkpoint.
...
const (
	// ProjectNameTag is a tag that represents the name of a project (coresponds to the `name` property of Khulnasoft.yaml).
	ProjectNameTag StackTagName = "khulnasoft:project"
	// ProjectRuntimeTag is a tag that represents the runtime of a project (the `runtime` property of Khulnasoft.yaml).
	ProjectRuntimeTag StackTagName = "khulnasoft:runtime"
	// ProjectDescriptionTag is a tag that represents the description of a project (Khulnasoft.yaml's `description`).
...
const (
	// KhulnasoftTagsConfigKey sets additional tags for a stack on a deployment. This is additive to any
	// tags that are already set on the stack.
	KhulnasoftTagsConfigKey string = "khulnasoft:tags"
)

// Stack describes a Stack running on a Khulnasoft Cloud.
type Stack struct {
...
// OperationStatus describes the state of an operation being performed on a Khulnasoft stack.
