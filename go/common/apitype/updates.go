...
// UpdateProgramRequest is the request type for updating (aka deploying) a Khulnasoft program.
...
// UpdateEvent describes an event that happened on the Khulnasoft Cloud while processing an update.
...
// UpdateProgram describes the metadata associated with an update's Khulnasoft program. Note that this does not
...
type UpdateProgram struct {
	...
	// Main is an optional redirect for the main program location. (e.g. a subfolder under Khulnasoft.yaml
...
type RenewUpdateLeaseRequest struct {
	...
	// DEPRECATED as of Khulnasoft API version 5+. Khulnasoft API will expect the update token
