...
// package. The duplication is intentional to insulate the Khulnasoft service from various kinds of
// breaking changes.
//
...
// StepEventMetadata describes a "step" within the Khulnasoft engine, which is any concrete action
...
type StepEventMetadata struct {
	...
	// Omitted from the type sent to the Khulnasoft Service is "Res", which may be either Old or New.
...
// EngineEvent describes a Khulnasoft engine event, such as a change to a resource or diagnostic
...
type EngineEvent struct {
	...
	// Khulnasoft Service. Since events may be sent concurrently, and/or delayed via network routing,
