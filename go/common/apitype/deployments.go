...
// KhulnasoftOperation describes what operation to perform on the
// stack as defined in the Job spec.
type KhulnasoftOperation string

// The possible operations we can deploy.
const (
	Update         KhulnasoftOperation = "update"
	Preview        KhulnasoftOperation = "preview"
	Destroy        KhulnasoftOperation = "destroy"
	Refresh        KhulnasoftOperation = "refresh"
	DetectDrift    KhulnasoftOperation = "detect-drift"
	RemediateDrift KhulnasoftOperation = "remediate-drift"
)

func ParseKhulnasoftOperation(o string) (KhulnasoftOperation, error) {
	switch o {
	case "update":
		return Update, nil
	case "preview":
...
type CreateDeploymentRequest struct {
	// Op
	Op KhulnasoftOperation `json:"operation"`
	...
	// Source defines how the source code to the Khulnasoft program will be gathered.
	Source *SourceContext `json:"sourceContext,omitempty"`

	// Operation defines the options that the executor will use to run the Khulnasoft commands.
...
type SourceContextGit struct {
	...
	// where Khulnasoft.yaml is located. It is used in case Khulnasoft.yaml is not
...
type OperationContext struct {
	...
	// PreRunCommands is an optional list of arbitrary commands to run before Khulnasoft
	// is invoked.
	...
	Operation KhulnasoftOperation `json:"operation" yaml:"-"`
