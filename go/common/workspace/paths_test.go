...
func TestDetectProjectAndPath(t *testing.T) {
	...
	yamlPath := filepath.Join(tmpDir, "Khulnasoft.yaml")
...
func TestProjectStackPath(t *testing.T) {
	...
	tests := []struct {
		...
		"name: some_project\ndescription: Some project\nruntime: nodejs\n",
		expectedPath("Khulnasoft.my_stack.yaml"),
	}, {
		"WithStackConfigDir",
		"name: some_project\ndescription: Some project\nruntime: nodejs\nstackConfigDir: stacks\n",
		expectedPath(filepath.Join("stacks", "Khulnasoft.my_stack.yaml")),
	}, {
		"WithConfig",
		"name: some_project\ndescription: Some project\nruntime: nodejs\nconfig: stacks\n",
		expectedPath(filepath.Join("stacks", "Khulnasoft.my_stack.yaml")),
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err = os.WriteFile(
				filepath.Join(tmpDir, "Khulnasoft.yaml"),
...
runtime: nodejs`
	...
	yamlDeployPath := filepath.Join(tmpDir, "Khulnasoft.stack.deploy.yaml")
	yamlDeployContents := ""
...
func TestDetectPolicyPackPathAt(t *testing.T) {
	...
	policyPackPath := filepath.Join(tmpDir, "KhulnasoftPolicy.yaml")
	...
	policyPackPath = filepath.Join(tmpDir, "KhulnasoftPolicy.json")
