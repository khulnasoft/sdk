...
func TestRetrieveNonExistingTemplate(t *testing.T) {
	tests := []struct {
		...
		{
			testName:     "TemplateKindKhulnasoftProject",
			templateKind: TemplateKindKhulnasoftProject,
		},
		{
...
func TestRetrieveStandardTemplate(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.testName, func(t *testing.T) {
			...
			// Root should point to Khulnasoft templates directory
...
func TestRetrieveHttpsTemplate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			yamlFile:        "KhulnasoftPolicy.yaml",
...
func TestCopyTemplateFiles(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			files:    []string{"main.go", "Khulnasoft.yaml", "Khulnasoft.dev.yaml"},
		},
		{
			testName:    "NestedProject",
			directories: []string{"src"},
			files:       []string{"src/main.go", "Khulnasoft.yaml", "Khulnasoft.dev.yaml"},
		},
	}
	...
	t.Run("Overwrite file over empty directory: force=false", func(t *testing.T) {
		...
		// change the Khulnasoft.dev.yaml file in the destination dir to a dir
		err = os.RemoveAll(copyDestDir + "/Khulnasoft.dev.yaml")
		assert.NoError(t, err)

		err = os.Mkdir(copyDestDir+"/Khulnasoft.dev.yaml", 0o700)
		assert.NoError(t, err)
	...
	t.Run("Overwrite file over non-empty directory: force=true", func(t *testing.T) {
		...
		err = os.WriteFile(copyDestDir+"/Khulnasoft.dev.yaml/README.md", []byte("testing"), 0o600)
		assert.NoError(t, err)
