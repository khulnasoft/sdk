...
import (
	...
	"github.com/khulnasoft/sdk/go/common/util/gitutil"
...
const (
	...
	// a project directory.
	legacyKhulnasoftTemplateManifestFile = ".khulnasoft.template.yaml"
...
var (
	...
	// The Git URL for Khulnasoft Policy Pack templates
...
const (
	// TemplateKindKhulnasoftProject is a template for a Khulnasoft stack.
	TemplateKindKhulnasoftProject TemplateKind = 0
...
func (repo TemplateRepository) Templates() ([]Template, error) {
	...
	// See if there's a Khulnasoft.yaml in the directory.
	...
	// that contain a Khulnasoft.yaml.
...
func (repo TemplateRepository) PolicyTemplates() ([]PolicyPackTemplate, error) {
	...
	// See if there's a KhulnasoftPolicy.yaml in the directory.
	...
	// Otherwise, read all subdirectories to find the ones
	// that contain a KhulnasoftPolicy.yaml.
	infos, err := os.ReadDir(path)
	if err != nil {
		return nil, err
	}
...
type Template struct {
	Dir         string                                // The directory containing Khulnasoft.yaml.
...
type PolicyPackTemplate struct {
	Dir         string // The directory containing KhulnasoftPolicy.yaml.
	Name        string // The name of the template.
	Description string // Description of the template.
	Error       error  // Non-nil if the template is broken.
}
...
func RetrieveTemplates(ctx context.Context, templateNamePathOrURL string, offline bool,
	...
	return retrieveKhulnasoftTemplates(ctx, templateNamePathOrURL, offline, templateKind)
}
...
// retrieveKhulnasoftTemplates retrieves the "template repository" for Khulnasoft templates.
// Instead of retrieving to a temporary directory, the Khulnasoft templates are managed from
// ~/.khulnasoft/templates.
func retrieveKhulnasoftTemplates(
...
func LoadTemplate(path string) (Template, error) {
	...
	// TODO handle other extensions like Khulnasoft.yml and Khulnasoft.json?
	proj, err := LoadProject(filepath.Join(path, "Khulnasoft.yaml"))
	if err != nil {
		return Template{}, err
	}
		...
		func(entry os.DirEntry, source string, dest string) error {
			...
			if !isBinary(b) {
				...
				if strings.HasSuffix(source, ".yaml") {
					// Use the sanitized project name and description for the Khulnasoft.yaml file.
					name = yamlName
...
func LoadPolicyPackTemplate(path string) (PolicyPackTemplate, error) {
	...
	pack, err := LoadPolicyPack(filepath.Join(path, "KhulnasoftPolicy.yaml"))
	if err != nil {
		return PolicyPackTemplate{}, err
	}
...
func GetTemplateDir(templateKind TemplateKind) (string, error) {
	...
	if templateKind == TemplateKindPolicyPack {
		return GetKhulnasoftPath(TemplatePolicyDir)
	}

	// Use the classic template directory if there is no override.
	return GetKhulnasoftPath(TemplateDir)
}
...
func walkFiles(sourceDir string, destDir string, projectName string,
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			...
			if name == legacyKhulnasoftTemplateManifestFile {
				continue
			}
