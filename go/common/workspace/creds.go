...
// KhulnasoftCredentialsPathEnvVar is a path to the folder where credentials are stored.
...
//
//nolint:gosec
const KhulnasoftCredentialsPathEnvVar = "KHULNASOFT_CREDENTIALS_PATH"
...
// Account holds the information associated with a Khulnasoft account.
type Account struct {
...
// Credentials hold the information necessary for authenticating Khulnasoft Cloud API requests.  It contains
...
// getCredsFilePath returns the path to the Khulnasoft credentials file on disk, regardless of
// whether it exists or not.
func getCredsFilePath() (string, error) {
	// Allow the folder we use to store credentials to be overridden by tests
	khulnasoftFolder := os.Getenv(KhulnasoftCredentialsPathEnvVar)
	if khulnasoftFolder == "" {
		folder, err := GetKhulnasoftHomeDir()
		if err != nil {
...
func GetStoredCredentials() (Credentials, error) {
	...
	if err = json.Unmarshal(c, &creds); err != nil {
		return Credentials{}, fmt.Errorf("failed to read Khulnasoft credentials file. Please fix "+
...
func GetKhulnasoftConfig() (KhulnasoftConfig, error) {
	configFile, err := getConfigFilePath()
	if err != nil {
		return KhulnasoftConfig{}, err
	}
	...
	if err != nil {
		...
		return KhulnasoftConfig{}, fmt.Errorf("reading '%s': %w", configFile, err)
	}

	var config KhulnasoftConfig
	if err = json.Unmarshal(c, &config); err != nil {
		return KhulnasoftConfig{}, fmt.Errorf("failed to read Khulnasoft config file: %w", err)
	}

	return config, nil
}

func StoreKhulnasoftConfig(config KhulnasoftConfig) error {
	configFile, err := getConfigFilePath()
	if err != nil {
		return err
	}
...
func SetBackendConfigDefaultOrg(backendURL, defaultOrg string) error {
	config, err := GetKhulnasoftConfig()
	if err != nil && !os.IsNotExist(err) {
		return err
	}
	...
	return StoreKhulnasoftConfig(config)
}
