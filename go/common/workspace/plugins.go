...
import (
	...
	"github.com/khulnasoft/sdk/go/common/util/archive"
	...
	"github.com/khulnasoft/sdk/go/common/util/retry"
...
var enableLegacyPluginBehavior = os.Getenv("KHULNASOFT_ENABLE_LEGACY_PLUGIN_SEARCH") != ""
...
// getKhulnasoftSource can download a plugin from get.khulnasoft.com
type getKhulnasoftSource struct {
	name string
	kind apitype.PluginKind
}

func newGetKhulnasoftSource(name string, kind apitype.PluginKind) *getKhulnasoftSource {
	return &getKhulnasoftSource{name: name, kind: kind}
}

func (source *getKhulnasoftSource) Download(
	ctx context.Context,
	version semver.Version, opSy string, arch string,
...
func (source *fallbackSource) Download(
	...
	khulnasoft := newGetKhulnasoftSource(source.name, source.kind)
...
// format. Previous versions of Khulnasoft extracted the tarball to a temp directory first, and then renamed the temp
...
func (spec PluginSpec) InstallWithContext(ctx context.Context, content PluginContent, reinstall bool) error {
	...
	// Cleanup any temp dirs from failed installations of this plugin from previous versions of Khulnasoft.
	...
	proj, err := LoadPluginProject(filepath.Join(finalDir, "KhulnasoftPlugin.yaml"))
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("loading KhulnasoftPlugin.yaml: %w", err)
	}
	if proj != nil {
...
// cleanupTempDirs cleans up leftover temp dirs from failed installs with previous versions of Khulnasoft.
...
//
// Deprecated: PluginKind type was moved to "github.com/khulnasoft/sdk/go/common/apitype"
...
//
// Deprecated: IsPluginKind type was moved to "github.com/khulnasoft/sdk/go/common/apitype"
...
func GetPolicyDir(orgName string) (string, error) {
	return GetKhulnasoftPath(PolicyDir, orgName)
}
...
func GetPluginDir() (string, error) {
	return GetKhulnasoftPath(PluginDir)
}
...
// possible to opt out of this behavior by setting KHULNASOFT_IGNORE_AMBIENT_PLUGINS to any non-empty value.
...
// installingPluginRegexp matches the name of temporary folders. Previous versions of Khulnasoft first extracted
