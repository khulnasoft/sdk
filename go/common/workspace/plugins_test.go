...
import (
	...
	"github.com/khulnasoft/sdk/go/common/testing/diagtest"
...
func TestPluginDownload(t *testing.T) {
	...
	t.Run("Khulnasoft GitHub Releases", func(t *testing.T) {
	...
	t.Run("Private Khulnasoft GitHub Releases", func(t *testing.T) {
	...
	t.Run("Khulnasoft GitHub Releases With Checksum", func(t *testing.T) {
		t.Setenv("GITHUB_TOKEN", "")
		version := semver.MustParse("4.30.0")
		getHTTPResponse := func(req *http.Request) (io.ReadCloser, int64, error) {
			if req.URL.String() == "https://api.github.com/repos/khulnasoft/khulnasoft-mockdl/releases/tags/v4.30.0" {
...
func TestUnmarshalProjectWithProviderList(t *testing.T) {
	t.Parallel()
	tempdir := t.TempDir()
	pyaml := filepath.Join(tempdir, "Khulnasoft.yaml")

	// write to pyaml
	err := os.WriteFile(pyaml, []byte(`name: test-yaml
runtime: yaml
description: "Test Khulnasoft YAML"
plugins:
  providers:
  - name: aws
    version: 1.0.0
...
func TestBundledPluginSearch(t *testing.T) {
	...
	t.Setenv("KHULNASOFT_IGNORE_AMBIENT_PLUGINS", "false")
	...
	t.Setenv("KHULNASOFT_IGNORE_AMBIENT_PLUGINS", "true")
	path, err = GetPluginPath(d, apitype.LanguagePlugin, "nodejs", nil, nil)
	require.NoError(t, err)
...
func TestPluginInfoShimless(t *testing.T) {
	...
	require.NoError(t, err)
	err = os.WriteFile(filepath.Join(pluginPath, "KhulnasoftPlugin.yaml"), []byte(`runtime: nodejs`), 0o600)
	require.NoError(t, err)
