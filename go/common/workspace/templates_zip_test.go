...
func TestRetrieveZIPTemplates_FailsWhenKhulnasoftYAMLIsMissing(t *testing.T) {
	// Arrange.
	...
	for path := range cases {
		...
		// Assert.
		assert.ErrorContains(t, err, "template does not contain a Khulnasoft.yaml file")
	}
}

//nolint:paralleltest // uses shared server URL
func TestRetrieveZIPTemplates_SucceedsWhenKhulnasoftYAMLIsPresent(t *testing.T) {
	// Arrange.
	cases := map[string][]string{
		"just-khulnasoft-yaml.zip":                    {"Khulnasoft.yaml"},
		"khulnasoft-yaml-and-flat-files.zip":          {"Khulnasoft.yaml", "foo"},
		"khulnasoft-yaml-and-nested-files.zip":        {"Khulnasoft.yaml", "bar/baz"},
		"khulnasoft-yaml-and-mixture.zip":             {"Khulnasoft.yaml", "foo", "bar/baz"},
		"khulnasoft-yaml-at-top-level-and-nested.zip": {"Khulnasoft.yaml", "foo", "bar/Khulnasoft.yaml"},
	}
