...
func RetrieveZIPTemplateFolder(templateURL *url.URL, tempDir string) (string, error) {
	...
	hasKhulnasoftYAML := false
	for _, file := range archive.File {
		if !file.FileInfo().IsDir() && file.FileInfo().Name() == "Khulnasoft.yaml" {
			hasKhulnasoftYAML = true
			break
		}
	}
	if !hasKhulnasoftYAML {
		return "", errors.New("template does not contain a Khulnasoft.yaml file")
	}
