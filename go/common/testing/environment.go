...
const (
	//nolint:gosec
	khulnasoftCredentialsPathEnvVar = "KHULNASOFT_CREDENTIALS_PATH"
)
...
type Environment struct {
	*testing.T
	...
	// HomePath is the KHULNASOFT_HOME directory for the environment
	HomePath string
	...
	Passphrase string
	// Set to true to turn off setting KHULNASOFT_CONFIG_PASSPHRASE.
	NoPassphrase bool
	// Set to true to use the local Khulnasoft dev build from ~/.khulnasoft-dev/bin/khulnasoft which get from `make install`
	UseLocalKhulnasoftBuild bool
...
func NewEnvironment(t *testing.T) *Environment {
	...
	// We always use a clean KHULNASOFT_HOME for each environment to avoid any potential conflicts with plugins or config.
	home, err := os.MkdirTemp("", "test-env-home")
	assert.NoError(t, err, "creating temp KHULNASOFT_HOME directory")
...
func (e *Environment) SetupCommandIn(dir string, command string, args ...string) *exec.Cmd {
	...
	cmd.Env = append(cmd.Env, "KHULNASOFT_DEBUG_COMMANDS=true")
	cmd.Env = append(cmd.Env, "KHULNASOFT_HOME="+e.HomePath)
	if !e.NoPassphrase {
		cmd.Env = append(cmd.Env, "KHULNASOFT_CONFIG_PASSPHRASE="+passphrase)
	}
	if e.Backend != "" {
		cmd.Env = append(cmd.Env, "KHULNASOFT_BACKEND_URL="+e.Backend)
	}
	// According to https://pkg.go.dev/os/exec#Cmd.Env:
	//     If Env contains duplicate environment keys, only the last
	//     value in the slice for each duplicate key is used.
