...
func TestEnvOverrideGetCommandResults(t *testing.T) {
	...
	checkDebug := func(expect string) {
		stdout, stderr, err := e.GetCommandResults("bash", "-c", "echo $KHULNASOFT_DEBUG_COMMANDS")
		stdout = strings.TrimSuffix(stdout, "\n")
		assert.NoError(t, err)
		assert.Empty(t, stderr)
		assert.Equal(t, expect, stdout)
	}
	// We default KHULNASOFT_DEBUG_COMMANDS to true
	checkDebug("true")
	// We can override the default
	e.SetEnvVars("KHULNASOFT_DEBUG_COMMANDS=false")
	checkDebug("false")
}
