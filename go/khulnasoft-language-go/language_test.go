...
import (
	...
	testingrpc "github.com/khulnasoft/sdk/proto/go/testing"
...
func (e *hostEngine) Log(_ context.Context, req *khulnasoftrpc.LogRequest) (*pbempty.Empty, error) {
	...
	message := req.Message
	if os.Getenv("KHULNASOFT_LANGUAGE_TEST_SHOW_FULL_OUTPUT") != "true" {
		// Cut down logs so they don't overwhelm the test output
		if len(message) > 1024 {
			message = message[:1024] + "... (truncated, run with KHULNASOFT_LANGUAGE_TEST_SHOW_FULL_OUTPUT=true to see full logs))"
		}
	}
...
func runEngine(t *testing.T) string {
	// Run a gRPC server that implements the Khulnasoft engine RPC interface. But all we do is forward logs on to T.
