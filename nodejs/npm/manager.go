...
// packaging Khulnasoft programs, and executing Node in the context of
...
// generate useful data. If the `KHULNASOFT_PREFER_YARN` environment variable is set, `yarn pack` is run
// instead of `npm pack`.
...
// app located there. If the `KHULNASOFT_PREFER_YARN` environment variable is set, `yarn install` is used
...
// manager is used. Otherwise, if the `KHULNASOFT_PREFER_YARN` environment variable is set, or if
...
func ResolvePackageManager(packagemanager PackageManagerType, pwd string) (PackageManager, error) {
	...
	// Prefer yarn if KHULNASOFT_PREFER_YARN is truthy, or if yarn.lock exists.
...
// preferYarn returns true if the `KHULNASOFT_PREFER_YARN` environment variable is set.
func preferYarn() bool {
	return cmdutil.IsTruthy(os.Getenv("KHULNASOFT_PREFER_YARN"))
}
