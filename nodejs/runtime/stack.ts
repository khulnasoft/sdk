 ...
 * of Khulnasoft resources allocated by a deployment. This must be kept up to date
 * with `github.com/khulnasoft/sdk/go/common/resource/stack.RootStackType`.
 */
export const rootKhulnasoftStackTypeName = "khulnasoft:khulnasoft:Stack";

/**
 * Creates a new Khulnasoft stack resource and executes the callback inside of it.
 * Any outputs returned by the callback will be stored as output properties on
 * this resulting Stack object.
 */
export function runInKhulnasoftStack(init: () => Promise<any>): Promise<Inputs | undefined> {
    if (!isQueryMode()) {
...
/**
 * {@link Stack} is the root resource for a Khulnasoft stack. Before invoking the
 * `init` callback, it registers itself as the root resource with the Khulnasoft
 * engine.
 */
export class Stack extends ComponentResource<Inputs> {
    ...
    constructor(init: () => Promise<Inputs>) {
        ...
        super(rootKhulnasoftStackTypeName, `${getProject()}-${getStack()}`, { init });
     ...
     *
     * @param args.init
     *  The callback to run in the context of this Khulnasoft stack
     */
...
async function massageComplex(prop: any, objectStack: any[]): Promise<any> {
    ...
    if (Resource.isInstance(prop)) {
        ...
        //
        // In preview only, we mark the POJO with "@isKhulnasoftResource" to indicate that it is derived
        ...
        return !isDryRun() ? pojo : { ...pojo, "@isKhulnasoftResource": true };
    }
...
/**
 * Add a transformation to all future resources constructed in this Khulnasoft
 * stack.
 */
...
export function registerResourceTransform(t: ResourceTransform): void {
    if (!getStore().supportsTransforms) {
        throw new Error("The Khulnasoft CLI does not support transforms. Please update the Khulnasoft CLI");
    }
...
/**
 * Add a transformation to all future invoke calls in this Khulnasoft stack.
 */
