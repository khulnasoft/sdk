 ...
 * @internal
 */
export const debugPromiseLeaks: boolean = !!process.env.KHULNASOFT_DEBUG_PROMISE_LEAKS;
...
export function leakedPromises(): [Set<Promise<any>>, string] {
    ...
    const message =
        leaked.size === 0
            ? ""
            : `The Khulnasoft runtime detected that ${leaked.size} ${promisePlural} still active\n` +
              "at the time that the process exited. There are a few ways that this can occur:\n" +
              "  * Not using `await` or `.then` on a Promise returned from a Khulnasoft API\n" +
              "  * Introducing a cyclic dependency between two Khulnasoft Resources\n" +
              "  * A bug in the Khulnasoft Runtime\n" +
              "\n" +
              "Leaving promises active is probably not what you want. If you are unsure about\n" +
              "why you are seeing this message, re-run your program " +
              "with the `KHULNASOFT_DEBUG_PROMISE_LEAKS`\n" +
              "environment variable. The Khulnasoft runtime will then print out additional\n" +
