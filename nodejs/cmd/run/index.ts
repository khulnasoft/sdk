...
function main(args: string[]): void {
    ...
    addToEnvIfDefined("KHULNASOFT_NODEJS_ORGANIZATION", argv["organization"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_PROJECT", argv["project"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_STACK", argv["stack"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_DRY_RUN", argv["dry-run"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_QUERY_MODE", argv["query-mode"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_PARALLEL", argv["parallel"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_MONITOR", argv["monitor"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_ENGINE", argv["engine"]);
    addToEnvIfDefined("KHULNASOFT_NODEJS_SYNC", argv["sync"]);
    ...
    v8Hooks.isInitializedAsync().then(() => {
        ...
        // when the user's program completes successfully, set programRunning back to false.  That way, if the Khulnasoft
