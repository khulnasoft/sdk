 ...
 * @param programPath the path to the Khulnasoft program; this is the project "main" directory,
...
async function npmPackageRootFromProgramPath(programPath: string): Promise<string> {
    ...
    if (pkgDir === undefined) {
        log.warn(
            "Could not find a package.json file for the program. Using the Khulnasoft program directory as the project root.",
        );
...
async function throwOrPrintModuleLoadError(program: string, error: Error): Promise<void> {
    ...
    // Khulnasoft CLI doesn't run build scripts before running the program so call that out
    // explicitly.
    ...
    if ("build" in scripts) {
        ...
        console.error(
            "Khulnasoft does not run build scripts before running your program. " +
    ...
    console.error("  * Khulnasoft encountered an unexpected error.");
...
function tracingIsEnabled(tracingUrl: string | boolean): boolean {
    ...
    const experimental = process.env["KHULNASOFT_EXPERIMENTAL"] ?? "";
...
export async function run(
    ...
    const typeScript: boolean = process.env["KHULNASOFT_NODEJS_TYPESCRIPT"] === "true";

    // We provide reasonable defaults for many ts options, meaning you don't need to have a tsconfig.json present
    // if you want to use TypeScript with Khulnasoft. However, ts-node's default behavior is to walk up from the cwd to
    ...
    const tsConfigPath: string = process.env["KHULNASOFT_NODEJS_TSCONFIG_PATH"] ?? defaultTsConfigPath;
    const skipProject = !fs.existsSync(tsConfigPath);

    span.setAttribute("typescript-enabled", typeScript);
    if (typeScript) {
        const transpileOnly = (process.env["KHULNASOFT_NODEJS_TRANSPILE_ONLY"] ?? "false") === "true";
        ...
        tsn.register({
            ...
            transpileOnly,
            // KHULNASOFT_NODEJS_TSCONFIG_PATH might be set to a config file such as "tsconfig.khulnasoft.yaml" which
...
${defaultMessage}`,
    ...
    const runProgram = async () => {
        ...
        try {
            ...
            // If there is no entrypoint set in Khulnasoft.yaml via the main
            ...
            if (await containsTSAndJSModules(program)) {
                log.warn(
                    "Found a TypeScript project containing an index.js file and no explicit entrypoint in Khulnasoft.yaml - Khulnasoft will use index.js",
                );
            }
    ...
    const stackOutputs = await stack.runInKhulnasoftStack(runProgram);
