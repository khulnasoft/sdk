...
function throwOrPrintModuleLoadError(program: string, error: Error): void {
    ...
    // Is there a build script associated with this program? It's a little confusing that the Khulnasoft
...
export function run(opts: RunOpts): Promise<Record<string, any> | undefined> | Promise<void> {
    ...
    // tsconfig.json present if you want to use TypeScript with Khulnasoft. However, ts-node's default
...
${errMsg}`,
    ...
    return opts.runInStack ? stack.runInKhulnasoftStack(runProgram) : runProgram();
}
