...
func TestGetRequiredPackages(t *testing.T) {
	...
	host := &nodeLanguageHost{}
	resp, err := host.GetRequiredPackages(context.Background(), &khulnasoftrpc.GetRequiredPackagesRequest{
		Info: &khulnasoftrpc.ProgramInfo{
			RootDirectory:    dir,
...
func TestGetRequiredPackagesNestedPolicyPack(t *testing.T) {
	...
	files := []struct {
		...
		{
			filepath.Join(dir, "policy", "KhulnasoftPolicy.yaml"),
			`name: my-policy`,
		},
		{
  ...
  dependencies:
	...
	t.Run("With package.json and yarn.lock in parent dir", func(t *testing.T) {
		...
		testDir := setupFiles(t, []filePathAndContents{
			...
			{
				path:    filepath.Join("subdir", "Khulnasoft.yaml"),
				content: `name: test`,
			},
			{
				path: "yarn.lock",
