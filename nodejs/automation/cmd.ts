...
export interface KhulnasoftCommandOptions {
    /**
    ...
    root?: string;
    /**
     * Skips the minimum CLI version check, see `KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK`.
     */
    skipVersionCheck?: boolean;
}

export class KhulnasoftCommand {
    private constructor(
    ...
    /**
     * Get a new Khulnasoft instance that uses the installation in `opts.root`.
     ...
     * `KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK` or setting
     ...
     */
    static async get(opts?: KhulnasoftCommandOptions): Promise<KhulnasoftCommand> {
        ...
        const version = parseAndValidateKhulnasoftVersion(min, stdout.trim(), skipVersionCheck);
        return new KhulnasoftCommand(command, version);
    }

    /**
     * Installs the Khulnasoft CLI.
     */
    static async install(opts?: KhulnasoftCommandOptions): Promise<KhulnasoftCommand> {
        const optsWithDefaults = withDefaults(opts);
        try {
            return await KhulnasoftCommand.get(optsWithDefaults);
        } catch (err) {
            // ignore
        }

        if (process.platform === "win32") {
            await KhulnasoftCommand.installWindows(optsWithDefaults);
        } else {
            await KhulnasoftCommand.installPosix(optsWithDefaults);
        }

        return await KhulnasoftCommand.get(optsWithDefaults);
    }

    private static async installWindows(opts: Required<KhulnasoftCommandOptions>): Promise<void> {
    ...
    private static async installPosix(opts: Required<KhulnasoftCommandOptions>): Promise<void> {
...
function withDefaults(opts?: KhulnasoftCommandOptions): Required<KhulnasoftCommandOptions> {
...
/**
 * @internal
 * Throws an error if the Khulnasoft CLI version is not valid.
 *
 * @param minVersion The minimum acceptable version of the Khulnasoft CLI.
 ...
 */
export function parseAndValidateKhulnasoftVersion(
    ...
    if (version == null) {
        throw new Error(
            `Failed to parse Khulnasoft CLI version. This is probably an internal error. You can override this by setting "${SKIP_VERSION_CHECK_VAR}" to "true".`,
        );
    }
    if (minVersion.major < version.major) {
        throw new Error(
            `Major version mismatch. You are using Khulnasoft CLI version ${currentVersion} with Automation SDK v${minVersion.major}. Please update the SDK.`,
        );
    }
    if (minVersion.compare(version) === 1) {
        throw new Error(
            `Minimum version requirement failed. The minimum CLI version requirement is ${minVersion.toString()}, your current CLI version is ${currentVersion}. Please update the Khulnasoft CLI.`,
        );
    }
    return version;
}
