...
import { CommandResult, KhulnasoftCommand } from "./cmd";
...
import { Deployment, PluginInfo, KhulnasoftFn, StackSummary, WhoAmIResult, Workspace } from "./workspace";

const SKIP_VERSION_CHECK_VAR = "KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK";

/**
 * {@link LocalWorkspace} is a default implementation of the {@link Workspace} interface.
 *
 * A {@link Workspace} is the execution context containing a single Khulnasoft
 ...
 * installation, environment configuration (`$KHULNASOFT_HOME`), and creation,
 * deletion, and listing of Stacks.
 *
 * {@link LocalWorkspace} relies on `Khulnasoft.yaml` and `Khulnasoft.<stack>.yaml` as
 * the intermediate format for Project and Stack settings. Modifying the
 * workspace's {@link ProjectSettings} will alter the workspace's `Khulnasoft.yaml`
 * file, and setting config on a Stack will modify the relevant
 * `Khulnasoft.<stack>.yaml` file. This is identical to the behavior of Khulnasoft CLI
 * driven workspaces.
 *
 * @alpha
 */
export class LocalWorkspace implements Workspace {
    /**
     * The working directory to run Khulnasoft CLI commands in.
     */
    readonly workDir: string;

    /**
     * The directory override for CLI metadata if set. This customizes the
     * location of `$KHULNASOFT_HOME` where metadata is stored and plugins are
     * installed.
     */
    ...
    private _khulnasoftCommand?: KhulnasoftCommand;

    /**
     * The underlying Khulnasoft CLI.
     */
    public get khulnasoftCommand(): KhulnasoftCommand {
        if (this._khulnasoftCommand === undefined) {
            throw new Error("Failed to get Khulnasoft CLI");
        }
        return this._khulnasoftCommand;
    }

    /**
     * The inline program {@link KhulnasoftFn} to be used for preview/update
     * operations if any. If none is specified, the stack will refer to
     * {@link ProjectSettings} for this information.
     */
    program?: KhulnasoftFn;

    /**
     * Environment values scoped to the current workspace. These will be supplied to every Khulnasoft command.
     */
    ...
    /**
     * The version of the underlying Khulnasoft CLI/engine.
     *
    ...
    public get khulnasoftVersion(): string {
        if (this._khulnasoftVersion === undefined) {
            throw new Error(`Failed to get Khulnasoft version`);
        }
    ...
    /**
     * An optional list of arbitrary commands to run before the remote Khulnasoft operation is invoked.
     */
    private remotePreRunCommands?: string[];

    /**
     * The environment variables to pass along when running remote Khulnasoft operations.
     */
     ...
     * Khulnasoft CLI program from the specified working directory. This is a way to
     * create drivers on top of pre-existing Khulnasoft programs. This workspace
     * will pick up any available settings files (`Khulnasoft.yaml`,
     * `Khulnasoft.<stack>.yaml`).
     *
     * @param args
     *  A set of arguments to initialize a stack with a pre-configured Khulnasoft
     ...
     * specified inline (in process) Khulnasoft program. This program is fully
     ...
     *  {@link KhulnasoftFn} program that runs in process.
     * @param opts
     *  Additional customizations to be applied to the Workspace.
     */
     ...
     * the specified inline (in process) Khulnasoft program. This program is fully
     * debuggable and runs in process. If no Project option is specified,
     * default project settings will be created on behalf of the user.
     * Similarly, unless a `workDir` option is specified, the working directory
     * will default to a new temporary directory provided by the OS.
     *
     * @param args
     *  A set of arguments to initialize a Stack with and inline `KhulnasoftFn`
     ...
     * utilizing the specified inline (in process) Khulnasoft CLI program. This
     ...
     * LocalWorkspace} utilizing the specified inline Khulnasoft CLI program. This
    ...
    private constructor(opts?: LocalWorkspaceOptions) {
        ...
        const khulnasoftCommand = opts?.khulnasoftCommand
            ...
            : KhulnasoftCommand.get({ skipVersionCheck });
     ...
     * {@link LocalWorkspace} reads settings from the `Khulnasoft.yaml`
     ...
     * old. {@link LocalWorkspace} writes this value to a `Khulnasoft.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param settings
    ...
    async saveProjectSettings(settings: ProjectSettings): Promise<void> {
        ...
        for (const ext of settingsExtensions) {
            const testPath = upath.joinSafe(this.workDir, `Khulnasoft${ext}`);
            if (fs.existsSync(testPath)) {
                foundExt = ext;
                break;
            }
        }
        const path = upath.joinSafe(this.workDir, `Khulnasoft${foundExt}`);
        let contents;
        if (foundExt === ".json") {
     ...
     * `Khulnasoft.<stack>.yaml` file in `Workspace.WorkDir()`.
     *
     * @param stackName
    ...
    async stackSettings(stackName: string): Promise<StackSettings> {
        ...
        for (const ext of settingsExtensions) {
            ...
            const path = upath.joinSafe(this.workDir, `Khulnasoft.${stackSettingsName}${ext}`);
     ...
     * name. {@link LocalWorkspace} writes this value to a `Khulnasoft.<stack>.yaml`
    ...
    async saveStackSettings(stackName: string, settings: StackSettings): Promise<void> {
        ...
        let foundExt = ".yaml";
        for (const ext of settingsExtensions) {
            const testPath = upath.joinSafe(this.workDir, `Khulnasoft.${stackSettingsName}${ext}`);
            if (fs.existsSync(testPath)) {
                foundExt = ext;
                break;
            }
        }
        const path = upath.joinSafe(this.workDir, `Khulnasoft.${stackSettingsName}${foundExt}`);
    ...
    async createStack(stackName: string): Promise<void> {
        ...
        await this.runKhulnasoftCmd(args);
    }
    ...
    async addEnvironments(stackName: string, ...environments: string[]): Promise<void> {
        ...
        if (ver.compare("3.95.0") < 0) {
            throw new Error(`addEnvironments requires Khulnasoft version >= 3.95.0`);
        }

        await this.runKhulnasoftCmd(["config", "env", "add", ...environments, "--stack", stackName, "--yes"]);
    }
    ...
    async listEnvironments(stackName: string): Promise<string[]> {
        ...
        if (ver.compare("3.99.0") < 0) {
            throw new Error(`listEnvironments requires Khulnasoft version >= 3.99.0`);
        }

        const result = await this.runKhulnasoftCmd(["config", "env", "ls", "--stack", stackName, "--json"]);
    ...
    async removeEnvironment(stackName: string, environment: string): Promise<void> {
        ...
        // 3.95 added this command (https://github.com/khulnasoft/khulnasoft/releases/tag/v3.95.0)
        if (ver.compare("3.95.0") < 0) {
            throw new Error(`removeEnvironments requires Khulnasoft version >= 3.95.0`);
        }

        await this.runKhulnasoftCmd(["config", "env", "rm", environment, "--stack", stackName, "--yes"]);
    }

    /**
     * Returns the value associated with the specified stack name and key,
     * scoped to the current workspace. {@link LocalWorkspace} reads this config
     * from the matching `Khulnasoft.stack.yaml` file.
     *
     * @param stackName
    ...
    async getConfig(stackName: string, key: string, path?: boolean): Promise<ConfigValue> {
        ...
        const result = await this.runKhulnasoftCmd(args);
        return JSON.parse(result.stdout);
    }
     ...
     * matching `Khulnasoft.stack.yaml` file.
     *
     * @param stackName
     *  The stack to read config from
     */
    async getAllConfig(stackName: string): Promise<ConfigMap> {
        const result = await this.runKhulnasoftCmd(["config", "--show-secrets", "--json", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }

    /**
     * Sets the specified key-value pair on the provided stack name. {@link
     * LocalWorkspace} writes this value to the matching `Khulnasoft.<stack>.yaml`
     * file in `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
     ...
     * remove any matching values in the `Khulnasoft.<stack>.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
     ...
     * Will remove any matching values in the `Khulnasoft.<stack>.yaml` file in
     * `Workspace.WorkDir()`.
     *
     * @param stackName
     *  The stack to operate on
    ...
    async refreshConfig(stackName: string): Promise<ConfigMap> {
        await this.runKhulnasoftCmd(["config", "refresh", "--force", "--stack", stackName]);
    ...
    async getTag(stackName: string, key: string): Promise<string> {
        const result = await this.runKhulnasoftCmd(["stack", "tag", "get", key, "--stack", stackName]);
    ...
    async setTag(stackName: string, key: string, value: string): Promise<void> {
        await this.runKhulnasoftCmd(["stack", "tag", "set", key, value, "--stack", stackName]);
    }
    ...
    async removeTag(stackName: string, key: string): Promise<void> {
        await this.runKhulnasoftCmd(["stack", "tag", "rm", key, "--stack", stackName]);
    }
    ...
    async listTags(stackName: string): Promise<TagMap> {
        const result = await this.runKhulnasoftCmd(["stack", "tag", "ls", "--json", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }
    ...
    async whoAmI(): Promise<WhoAmIResult> {
        ...
        if (ver.compare("3.58.0") >= 0) {
            const result = await this.runKhulnasoftCmd(["whoami", "--json"]);
            return JSON.parse(result.stdout);
        } else {
            const result = await this.runKhulnasoftCmd(["whoami"]);
     ...
     * present in the workspace as `Khulnasoft.<stack>.yaml` files.
     *
     * @param opts
    ...
    async install(opts?: InstallOptions): Promise<void> {
        ...
        if (ver.compare("3.91.0") < 0) {
            // Khulnasoft 3.91.0 added the `khulnasoft install` command.
            // https://github.com/khulnasoft/khulnasoft/releases/tag/v3.91.0
            throw new Error(`khulnasoft install requires Khulnasoft version >= 3.91.0`);
        }
        ...
        if (opts?.useLanguageVersionTools) {
            if (ver.compare("3.130.0") < 0) {
                ...
                throw new Error(`useLanguageVersionTools requires Khulnasoft version >= 3.130.0`);
            }
        ...
        await this.runKhulnasoftCmd(["install", ...args]);
    }
    ...
    async installPlugin(name: string, version: string, kind = "resource"): Promise<void> {
        await this.runKhulnasoftCmd(["plugin", "install", kind, name, version]);
    }
    ...
    async installPluginFromServer(name: string, version: string, server: string): Promise<void> {
        await this.runKhulnasoftCmd(["plugin", "install", "resource", name, version, "--server", server]);
    }
    ...
    async listPlugins(): Promise<PluginInfo[]> {
        const result = await this.runKhulnasoftCmd(["plugin", "ls", "--json"]);
    ...
    async exportStack(stackName: string): Promise<Deployment> {
        const result = await this.runKhulnasoftCmd(["stack", "export", "--show-secrets", "--stack", stackName]);
        return JSON.parse(result.stdout);
    }
    ...
    async importStack(stackName: string, state: Deployment): Promise<void> {
        ...
        await this.runKhulnasoftCmd(["stack", "import", "--file", filepath, "--stack", stackName]);
    ...
    async stackOutputs(stackName: string): Promise<OutputMap> {
        ...
        const maskedResult = await this.runKhulnasoftCmd(["stack", "output", "--json", "--stack", stackName]);
        const plaintextResult = await this.runKhulnasoftCmd([
            "stack",
            "output",
            "--json",
            "--show-secrets",
     ...
     * create/modify a `Khulnasoft.stack.yaml`) {@link LocalWorkspace} does not
    ...
    private async checkRemoteSupport() {
        ...
        if (!optOut && this.isRemote) {
            ...
            const previewResult = await this.runKhulnasoftCmd(["preview", "--help"]);
            const previewOutput = previewResult.stdout.trim();
            if (!previewOutput.includes("--remote")) {
                throw new Error("The Khulnasoft CLI does not support remote operations. Please upgrade.");
            }
        }
    }

    private async runKhulnasoftCmd(args: string[]): Promise<CommandResult> {
        let envs: { [key: string]: string } = {};
        if (this.khulnasoftHome) {
            envs["KHULNASOFT_HOME"] = this.khulnasoftHome;
        }
        if (this.isRemote) {
            envs["KHULNASOFT_EXPERIMENTAL"] = "true";
        }
...
/**
 * Description of a stack backed by an inline (in process) Khulnasoft program.
 */
export interface InlineProgramArgs {
    ...
    /**
     * The inline (in-process) Khulnasoft program to use with update and preview operations.
     */
    program: KhulnasoftFn;
}

/**
 * Description of a stack backed by pre-existing local Khulnasoft CLI program.
 */
...
export interface LocalWorkspaceOptions {
    /**
     * The directory to run Khulnasoft commands and read settings (`Khulnasoft.yaml` and
     * `Khulnasoft.<stack>.yaml`).
     */
    workDir?: string;
    ...
    /**
     * An optional list of arbitrary commands to run before a remote Khulnasoft operation is invoked.
     *
     * @internal
     */
...
function loadProjectSettings(workDir: string) {
    for (const ext of settingsExtensions) {
        const isJSON = ext === ".json";
        const path = upath.joinSafe(workDir, `Khulnasoft${ext}`);
        if (!fs.existsSync(path)) {
            continue;
        }
        const contents = fs.readFileSync(path).toString();
...
export interface RemoveOptions {
    ...
    /**
     * Do not delete the corresponding Khulnasoft.<stack-name>.yaml configuration file for the stack
     */
