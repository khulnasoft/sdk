 ...
 * Khulnasoft application. Specifically, an {@link Output} holds onto a piece of
...
class OutputImpl<T> implements OutputInstance<T> {
     ...
     * to work even when multiple copies of the Khulnasoft SDK have been loaded into
     * the same process.
     */
    ...
    static async getPromisedValue<T>(promise: Promise<T>, withUnknowns?: boolean): Promise<T> {
        ...
        // preserves compatibility with earlier versions of the Khulnasoft SDK.
 ...
 * unknown property values received from the Khulnasoft engine as part of the result
...
export interface OutputInstance<T> {
     ...
     * and can be safely retrieved. During Khulnasoft deployment or preview
 ...
 * relationship between resources.
 *
 * An output is used in a Khulnasoft program differently depending on if the
 ...
 *
 * To ease with using outputs at deployment time, Khulnasoft will "lift" simple data
