...
class CustResource extends khulnasoft.CustomResource {
    constructor(name, opts) {
        super("test:index:CustResource", name, {}, opts);
    }
}

class CompResource extends khulnasoft.ComponentResource {
    constructor(name, opts) {
        super("test:index:CompResource", name, {}, opts);
        const data = this.getData();
        this.a = khulnasoft.output(data.then((d) => d.a));
        this.b = khulnasoft.output(data.then((d) => d.b));
        this.cust1 = khulnasoft.output(data.then((d) => d.cust1));
        this.cust2 = khulnasoft.output(data.then((d) => d.cust2));
    }
