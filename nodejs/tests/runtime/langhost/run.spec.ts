            ...
            log: (ctx: any, severity: number, message: string, urn: URN, streamId: number) => {
                assert.strictEqual(
                    ...
                    "Found a TypeScript project containing an index.js file and no explicit entrypoint" +
                        " in Khulnasoft.yaml - Khulnasoft will use index.js",
                );
            },
        },
        unusual_alias_names: {
            ...
            for (const dryrun of [true, false]) {
                ...
                const monitor = await createMockEngineAsync(
                    ...
                    (call: any, callback: any) => {
                        ...
                        if (req.getType() !== runtime.rootKhulnasoftStackTypeName || opts.showRootResourceRegistration) {
...
function serveLanguageHostProcess(engineAddr: string): { proc: childProcess.ChildProcess; addr: Promise<string> } {
    ...
    //
    // `khulnasoft-language-nodejs` allows us to set `KHULNASOFT_LANGUAGE_NODEJS_RUN_PATH` in the environment, and
    ...
    process.env.KHULNASOFT_LANGUAGE_NODEJS_RUN_PATH = path.normalize(path.join(__dirname, "..", "..", "..", "cmd", "run"));
