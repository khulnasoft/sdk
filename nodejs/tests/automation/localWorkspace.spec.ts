...
describe("LocalWorkspace", () => {
        ...
        for (const ext of ["yaml", "yml", "json"]) {
            const ws = await LocalWorkspace.create(
                withTestBackend(
                    ...
                    "testproj",
                    "A minimal Go Khulnasoft program",
                    "go",
                ),
            );
            ...
            assert.strictEqual(settings.description, "A minimal Go Khulnasoft program");
        }
    });
    ...
    describe("Tag methods: get/set/remove/list", () => {
        ...
        it("lists tag values", async () => {
            if (!process.env.KHULNASOFT_ACCESS_TOKEN) {
    ...
    describe("ListStack Methods", async () => {
        describe("ListStacks", async () => {
            ...
            it(`should use correct args for listStacks`, async () => {
                ...
                const workspace = await LocalWorkspace.create(
                    withTestBackend({
                        khulnasoftCommand: mockKhulnasoftCommand,
                    }),
                );
    ...
    // Regression test for https://github.com/khulnasoft/khulnasoft/issues/17613
    ...
    it("sets khulnasoft version when using a custom CLI instance", async () => {
        ...
        try {
            const cmd = await KhulnasoftCommand.get();
    ...
    it("throws when attempting to retrieve an invalid khulnasoft version", async () => {
        ...
        const ws = await LocalWorkspace.create(
            withTestBackend({
                ...
                envVars: {
                    KHULNASOFT_AUTOMATION_API_SKIP_VERSION_CHECK: "true",
                },
            }),
        );
    ...
    it(`correctly sets config on multiple stacks concurrently`, async () => {
        ...
        const ws = await LocalWorkspace.create({
            ...
            envVars: {
                KHULNASOFT_CONFIG_PASSPHRASE: "test",
            },
        });
 ...
 * either a file backend or a cloud backend, depending on whether KHULNASOFT_ACCESS_TOKEN
 ...
 * environment containing a test `KHULNASOFT_CONFIG_PASSPHRASE` variable suitable
