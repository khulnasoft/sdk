...
import { KhulnasoftCommand, parseAndValidateKhulnasoftVersion } from "../../automation";

describe("automation/cmd", () => {
    it("calls onOutput when provided to runKhulnasoftCmd", async () => {
        let output = "";
        let numCalls = 0;
        const khulnasoft = await KhulnasoftCommand.get();
    ...
    describe("CLI installation", () => {
        it("installs the requested version", async () => {
            ...
            try {
                const cmd = await KhulnasoftCommand.install({ root: tmpDir.name, version: new semver.SemVer("3.97.0") });
        ...
        it("does not re-install the version if it already exists", async () => {
            const tmpDir = tmp.dirSync({ prefix: "automation-test-", unsafeCleanup: true });
            try {
                await KhulnasoftCommand.install({ root: tmpDir.name, version: new semver.SemVer("3.97.0") });
        ...
        it("defaults to $HOME/.khulnasoft/versions/$VERSION if no root is provided", async () => {
            ...
            const cmd = await KhulnasoftCommand.install({ version });
            assert.doesNotThrow(() =>
        ...
        it("throws if the found version is not compatible with the requested version", async () => {
            ...
            await KhulnasoftCommand.install({ version: installedVersion });
            const requestedVersion = new semver.SemVer("3.98.0");
            assert.rejects(KhulnasoftCommand.get({ version: requestedVersion }));
            assert.doesNotReject(KhulnasoftCommand.get({ version: installedVersion, skipVersionCheck: true }));
        });
    });
    ...
    describe(`checkVersionIsValid`, () => {
        ...
        versionTests.forEach((test) => {
            it(`validates ${test.name} (${test.currentVersion})`, () => {
                const validate = () => parseAndValidateKhulnasoftVersion(minVersion, test.currentVersion, test.optOut);
                if (test.expectError) {
